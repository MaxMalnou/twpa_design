...
# fig_standard = results.plot(show_plot=True, max_mode_order_to_plot=4)

# ============================================================================
# Option 2: Recreate the exact same plot from results.plot() method
# ============================================================================
print("Recreating the exact same plot as results.plot()...")
...
gs = gridspec.GridSpec(4, 1, figure=fig_custom, height_ratios=[2, 1, 1, 1], hspace=0.5)
...
# Plot 1: S-parameters (exact same code as _plot_s_parameters)
ax1.plot(results.frequencies_GHz, 10*np.log10(results.S21), color=blue, linewidth=linewidth)
ax1.plot(results.frequencies_GHz, 10*np.log10(results.S12), color=red, linewidth=linewidth)
ax1.plot(results.frequencies_GHz, 10*np.log10(results.S11), color=green, linewidth=linewidth, alpha=0.7)
ax1.plot(results.frequencies_GHz, 10*np.log10(results.S22), color=orange, linewidth=linewidth, alpha=0.7)

# Add vertical line at pump frequency
# ax1.axvline(config.pump_freq_GHz, color=purple, linestyle=':', alpha=0.5)
...
ax1.set_title('S-Parameters', fontsize=fontsize_title)
...
# Plot 2: Forward idlers (exact same code as _plot_forward_idlers)
...
if results.idler_response.ndim > 1 and results.idler_response.shape[0] > 1:
    ...
    if len(n_to_mode_idx) > 0:
        ...
        for i, n in enumerate(plot_order):
            ...
            ax2.plot(results.frequencies_GHz, idler_response_dB, 
...
# ax2.set_ylabel(rf'$S_{{{config.output_port}{config.signal_port}}}$ [dB]', fontsize=fontsize)
ax2.set_ylabel(rf'$|S_{{{config.output_port}{config.signal_port}}}|(\mathrm{{sig}}\rightarrow\mathrm{{idl}})$ [dB]', fontsize=fontsize)
ax2.set_title('Forward Idlers', fontsize=fontsize_title)
...
# Plot 3: Backward idlers (exact same code as _plot_backward_idlers)
...
# ax3.set_ylabel(rf'$S_{{{config.signal_port}{config.output_port}}}$ [dB]', fontsize=fontsize)
ax3.set_ylabel(rf'$|S_{{{config.signal_port}{config.output_port}}}|(\mathrm{{sig}}\rightarrow\mathrm{{idl}})$ [dB]', fontsize=fontsize)
ax3.set_title('Backward Idlers', fontsize=fontsize_title)
